...
/////////////////////////////////////////// recrypt
#include <recrypt/recryptstate.h>
#include <recrypt/recryptDGP.h>
#include <libethereum/ChainParams.h>
#include <libethashseal/Ethash.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <recrypt/storageresults.h>


extern std::unique_ptr<RecryptState> globalState;
...
using ExtractRecryptTX = std::pair<std::vector<RecryptTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // recrypt
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashRecryptRoot,
    const dev::h256& oldHashStateRoot, const std::vector<RecryptTransaction>& transactions);
...
class RecryptTxConverter{

public:

    RecryptTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionRecryptTransactions(ExtractRecryptTX& recryptTx);
...
private:
    ...
    RecryptTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<RecryptTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
...
private:
    ...
    std::vector<RecryptTransaction> txs;
