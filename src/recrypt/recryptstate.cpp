...
#include "recryptstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

RecryptState::RecryptState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = RecryptState::openDB(_path + "/recryptDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

RecryptState::RecryptState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute RecryptState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, RecryptTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createRecryptAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            recrypt::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> RecryptState::vins() const // temp
{
...
void RecryptState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* RecryptState::vin(dev::Address const& _a) const
{
    return const_cast<RecryptState*>(this)->vin(_a);
}

Vin* RecryptState::vin(dev::Address const& _addr)
{
...
// void RecryptState::commit(CommitBehaviour _commitBehaviour)
// {
...
void RecryptState::kill(dev::Address _addr)
{
...
void RecryptState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address RecryptState::createRecryptAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void RecryptState::deleteAccounts(std::set<dev::Address>& addrs){
...
void RecryptState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void RecryptState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
