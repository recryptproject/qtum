...
void Shutdown()
{
    ...
    if (pwalletMain)
    {
        StakeRecrypts(false, pwalletMain);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/recryptproject/recrypt>";
    const std::string URL_WEBSITE = "<https://recrypt.org>";
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateRecrypt";
                ...
                boost::filesystem::path recryptStateDir = GetDataDir() / "stateRecrypt";

                bool fStatus = boost::filesystem::exists(recryptStateDir);
                const std::string dirRecrypt(recryptStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsRecryptstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<RecryptState>(new RecryptState(dev::u256(0), RecryptState::openDB(dirRecrypt, hashDB, dev::WithExisting::Trust), dirRecrypt, existsRecryptstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::recryptMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(recryptStateDir.string());
    ...
    if (!GetBoolArg("-staking", DEFAULT_STAKE))
        ...
        StakeRecrypts(true, pwalletMain);
#endif
    // ********************************************************* Step 12: finished
