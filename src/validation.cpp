...
#if defined(NDEBUG)
# error "Recrypt cannot be compiled without assertions."
#endif
...
std::unique_ptr<RecryptState> globalState;
...
const std::string strMessageMagic = "Recrypt Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // recrypt
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            RecryptDGP recryptDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = recryptDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = recryptDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            RecryptTxConverter converter(tx, NULL);
            ExtractRecryptTX resultConverter;
            if(!converter.extractionRecryptTransactions(resultConverter)){
            ...
            std::vector<RecryptTransaction> recryptTransactions = resultConverter.first;
            std::vector<EthTransactionParams> recryptETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(RecryptTransaction recryptTransaction : recryptTransactions){
                sumGas += recryptTransaction.gas() * recryptTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, recryptTransaction.gasPrice());
                } else {
                    txMinGasPrice = recryptTransaction.gasPrice();
                }
                VersionVM v = recryptTransaction.getVersion();
                if(v.format!=0)
                ...
                if(recryptTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(recryptTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(recryptTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += recryptTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)recryptTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(recryptETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > recryptTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s RECRYPT additional fees, %d delta bytes\n",
...
{
    if(nHeight <= (consensusParams.nLastPOWBlock - 1))
        return 20000 * COIN;
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // recrypt
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // recrypt
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    RecryptTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<RecryptTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path recryptDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(recryptDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(recryptDir.string(), std::ios::in | std::ios::out);
...
bool RecryptTxConverter::extractionRecryptTransactions(ExtractRecryptTX& recrypttx){
    std::vector<RecryptTransaction> resultTX;
    ...
    recrypttx = std::make_pair(resultTX, resultETP);
    return true;
}

bool RecryptTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool RecryptTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
RecryptTransaction RecryptTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    RecryptTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = RecryptTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = RecryptTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // recrypt
    RecryptDGP recryptDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setRecryptSchedule(recryptDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = recryptDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = recryptDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = recryptDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // recrypt
...
///////////////////////////////////////////////////////////////////////////////////////// recrypt
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            RecryptTxConverter convert(tx, &view, &block.vtx);

            ExtractRecryptTX resultConvertRecryptTX;
            if(!convert.extractionRecryptTransactions(resultConvertRecryptTX)){
            ...
            if(!CheckMinGasPrice(resultConvertRecryptTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertRecryptTX.first, blockGasLimit);
            ...
            for(RecryptTransaction& qtx : resultConvertRecryptTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertRecryptTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertRecryptTX.first[k].from(), resultConvertRecryptTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // recrypt
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // recrypt
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // recrypt
            globalState->setRootUTXO(oldHashUTXORoot); // recrypt
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // recrypt
        uint32_t sizeBlockDGP = recryptDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Recrypt PoS we need to sync this after every block to ensure txdb is populated for
